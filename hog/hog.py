...
def free_bacon(score):
    ...
    # BEGIN PROBLEM 2
    bacon_score = 2 * (score // 10) - (score % 10)
    return max(1, bacon_score)
    # END PROBLEM 2
...
def is_swap(player_score, opponent_score):
    ...
    """
    # BEGIN PROBLEM 4
    player_scoredif = abs(((player_score % 100) // 10) - (player_score % 10))
    opp_scoredif = abs(((opponent_score % 100) // 10) - (opponent_score % 10))
    if player_scoredif == opp_scoredif:
        return True
    else:
        return False
    # END PROBLEM 4
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while (score0 < goal) and (score1 < goal):
        if player == 0:
        	new_score = take_turn(strategy0(score0, score1), score1, dice)
        	score0 += new_score
        else:
        	new_score = take_turn(strategy1(score1, score0), score0, dice)
        	score1 += new_score
...
def announce_highest(who, previous_high=0, previous_score=0):
    ...
    def scores(score0, score1):
        ...
        if increase > previous_high:
            print(str(increase) + " point(s)! That's the biggest gain yet for Player " + str(who))
            new_high = increase
        if who == 0:
...
def max_scoring_num_rolls(dice=six_sided, num_samples=1000):
    ...
    """
    # BEGIN PROBLEM 9
    curr_rolls, max_ave, max_rolls = 1, 0, 0
    averaged_roll_dice = make_averaged(roll_dice, num_samples)
    while curr_rolls <= 10:
        curr_average = averaged_roll_dice(curr_rolls, dice)
        if curr_average > max_ave:
            max_rolls = curr_rolls
            max_ave = curr_average
        curr_rolls += 1
    return max_rolls
    # END PROBLEM 9
...
def swap_strategy(score, opponent_score, margin=8, num_rolls=4):
    ...
    if is_swap(new_score, opponent_score) and new_score < opponent_score:
        return 0
    elif bacon_strategy(score, opponent_score, margin, num_rolls) == 0 and is_swap(new_score, opponent_score) == False:
        return 0
    else:
        return num_rolls
