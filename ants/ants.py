        ...
        if insect.is_ant:
            ...
            if self.ant is insect:
                ...
                if hasattr(self.ant, 'is_container') and self.ant.is_container \
                    ...
                    if hasattr(self.ant.contained_ant, 'true_queen') and self.ant.contained_ant.true_queen:
                        return
...
class Bee(Insect):
    ...
    def action(self, colony):
        ...
        # BEGIN EC
            ...
            self.scare_effective -= 1
            if self.scare_effective == 0:
                self.direction = 1
        # END EC
        if self.blocked():
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        while place is not hive and dist <= self.max_range:
        	if dist < self.min_range or place.bees == []:
...
class HungryAnt(Ant):
    ...
    """
    name = 'Hungry'
    food_cost = 4   # OVERRIDE CLASS ATTRIBUTES HERE
    # BEGIN Problem 6
    implemented = True   # Change to True to view in the GUI
...
class NinjaAnt(Ant):
    ...
    def action(self, colony):
        ...
        for i in range(len(self.place.bees)):
            bee_copy[i].reduce_armor(self.damage)
            if bee_copy[i].armor != 0:
                new_bees.append(bee_copy[i])
...
# The WallAnt 
class WallAnt(Ant):
    """WallAnt protects the ant colony"""
    name = 'Wall'
    food_cost = 4
    implemented = True
...
# END Problem 13
    ...
    def __init__(self, armor=1):
        ...
        self.doubled_ants = []
        if not QueenAnt.first_exists:
            self.true_queen = True
            QueenAnt.first_exists = True
        # END Problem 13
    ...
    def action(self, colony):
        ...
        if not self.true_queen:
            ...
            while search_place is not None:
                if search_place.ant is not None:
                    if search_place.ant not in self.doubled_ants:
                        search_place.ant.damage *= 2
                        self.doubled_ants.append(search_place.ant)
                    if search_place.ant.is_container and search_place.ant.contained_ant and search_place.ant.contained_ant not in self.doubled_ants:
                        search_place.ant.contained_ant.damage *= 2
                        self.doubled_ants.append(search_place.ant.contained_ant)
