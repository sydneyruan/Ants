...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        # BEGIN PROBLEM 5
        curr_eval = lambda x: scheme_eval(x, env)
        oper = curr_eval(first)
        check_procedure(oper)
        return scheme_apply(oper, rest.map(curr_eval), env)
        # END PROBLEM 5
...
class BuiltinProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        try:
            ...
            raise SchemeError('arguments are not the right type: {0}'.format(args))
        # END PROBLEM 4
...
def do_define_form(expressions, env):
    ...
    to_bind = expressions.second.first
    if scheme_symbolp(target):
        check_form(expressions, 2, 2)
        # BEGIN PROBLEM 6
        env.define(target, scheme_eval(to_bind, env))
        return target
        # END PROBLEM 6
...
def make_let_frame(bindings, env):
    ...
    while bindings != nil:
        ...
        forms = Pair(bind_pair.first, forms)
        vals = Pair(scheme_eval(bind_pair.second.first, env), vals)
